...
- [ ] 1.3 Subscribe to the harness `tool_call` event (per installed `docs/extensions.md`, as pinned in design.md D1 and validate.md §1.3) for file-modifying tool activity and mark the workspace possibly-stale on first observed edit, with burst debouncing and zero detection spawns.
...
- [ ] 3.1 Implement once-per-condition-per-session notices (possibly-stale with `/cgc sync` hint; skipped-busy — shared condition key covering both lazy-sync busy skips and watcher-start-blocked-by-lock; sync-completed) through the session notice surface, never in the agent context.
