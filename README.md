...
[CodeGraphContext (CGC)](https://github.com/CodeGraphContext/CodeGraphContext): it keeps a
code-graph index healthy for every session and makes it effortless for the agent to use.

- **What it is** — a single pi extension that wraps the `cgc` CLI, evaluates the index
  lifecycle at session start, syncs drift automatically, and surfaces everything the agent
  needs: slash commands, a status widget, CLI-gap tools, and routing guidance.
- **Why it exists** — code-graph answers are only as good as the index behind them. Without
  a gate, agents either skip the graph (stale or missing index) or block on it (busy,
  corrupt). This extension makes the graph **always ready or honestly unavailable**, never
  silently wrong.

> [!NOTE]
> Graph relationship tools (`analyze_code_relationships`, `find_dead_code`,
> `find_code`, …) are registered by the **CGC MCP server**, not by this extension. The
...
3. Start (or restart) pi. On the first session in a repository you get exactly one
