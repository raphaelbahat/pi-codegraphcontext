...
- [ ] 2.1 Implement `proactive.driftSteers` (default false): one agent-facing steer per fresh→possibly-staleness episode, driven by freshness state transitions, naming `/cgc sync`; never fires when disabled.
- [ ] 2.2 Implement `proactive.resultAnnotations` (default false): one-line freshness annotation on extension-owned tool and command outputs only; semantics untouched; never on CGC MCP server results.
